...
public class UserService {
	...
	public User save(User user) {
		...
		User newUser = this.mediaUserRepository.save(user);
	...
	public List<User> saveSharedUsers(List<User> sharedUsers) {
		...
		for(User sharedUser: sharedUsers) {
			...
			if (user.isPresent()) {
				...
				this.subscriptionService.sendNotification(user.get(), sharedUser.getSharedLibraries().iterator().next());
				users.add(user.get());
			}
		}
	...
	public Set<User> getAllSharedUsersByAlbum(Long idAlbum) {
